 ...
 <div class="row">
  <router-link to="/role" class="btn btn-primary">All Role </router-link>
    ...
    <div class="row justify-content-center">
      ...
      <form class="user" @submit.prevent="roleUpdate" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-md-3" v-for="permission in listPermission" :key="permission.id">
                <div class="row">
                  <div class="col-md-3">
                    <div class="form-group">
                      <input type="checkbox" class="form-control" v-model="permissions" :value="permission.id" >
                    </div>
                  </div>
                  <div class="col-md-9">
                    {{ permission.name }}
...
<script type="text/javascript">
  ...
  export default {
    ...
    async created(){
      await this.$store.dispatch('role/fetchOne', this.$route.params.id)
      await this.$store.dispatch('permission/fetch')
      this.permissions = await this.loadCheck()
    },
  ...
  methods:{
    loadCheck() {
      let permissions = []
        for(let i = 0; i < this.$store.state.role.role.countRolePermissions; i++) {
          permissions[this.$store.state.role.role.rolePermissions[i]] = this.$store.state.role.role.rolePermissions[i]
        }
      return permissions
    },
    async roleUpdate(){
